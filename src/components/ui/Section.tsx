...
export function SectionHead({
  ...
  return (
    <div className={`${centro ? "mx-auto max-w-3xl text-center" : "max-w-2xl"} ${className}`}>
      ...
      <Reveal delay={0.08}>
        <h2 className="mt-5 text-balance text-4xl font-bold leading-[1.08] text-ink-900 sm:text-5xl lg:text-[3.4rem]">
          {titulo}{" "}
      ...
      {sub && (
        <Reveal delay={0.16}>
          <p className={`mt-5 text-lg leading-relaxed text-ink-500 ${centro ? "mx-auto max-w-2xl" : ""}`}>
            {sub}
          </p>
        </Reveal>
      )}
